...
# Get gRPC server address from environment variable or use localhost as default
GRPC_SERVER_ADDRESS = os.environ.get('GRPC_SERVER_ADDRESS', 'localhost')

def is_server_running():
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            return s.connect_ex((GRPC_SERVER_ADDRESS, 50051)) == 0
    except:
        return False

# Only try to start the server if we're using localhost
if GRPC_SERVER_ADDRESS == 'localhost' and not is_server_running():
    ...
    st.error(f"Could not connect to gRPC server at {GRPC_SERVER_ADDRESS}:50051. Please ensure it's running.")
    st.stop()

channel = grpc.insecure_channel(f'{GRPC_SERVER_ADDRESS}:50051')
